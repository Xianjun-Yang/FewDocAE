...
def main():
    ...
    parser.add_argument('--trainN', default=6, type=int,
                        help='N in train')
    ...
    parser.add_argument('--batch_size', default= 1, type=int,
                        help='batch size')
    parser.add_argument('--train_iter', default= 80000, type=int,
                        help='num of iters in training')
    parser.add_argument('--val_iter', default=4000, type=int,
                        help='num of iters in validation')
    parser.add_argument('--test_iter', default=4000, type=int,
                        help='num of iters in testing')
    parser.add_argument('--val_step', default=4000, type=int,
    ...
    parser.add_argument('--pretrain_ckpt', default= bertpath + 'led-base-16384',
    ...
    pretrain_ckpt = opt.pretrain_ckpt or 'allenai/led-base-16384'  # "allenai/led-base-16384", 'bert-base-uncased'
    word_encoder = BERTWordEncoder(
        pretrain_ckpt)
    tokenizer = LEDTokenizer.from_pretrained( pretrain_ckpt )  # "allenai/led-base-16384", 'bert-base-uncased'
    # LEDTokenizer, BertTokenizer
    print('Use episode data!')
    opt.train = f'../../project_data/DocEE/cross_setting/6w2d/train_6w2d.jsonl' # train_6w2d
    opt.dev = f'../../project_data/DocEE/cross_setting/6w2d/dev_6w2d.jsonl'
    opt.test = f'../../project_data/DocEE/cross_setting/6w2d/test_6w2d.jsonl' # test_6w2d

    if opt.mode == "supervised":
        print(
            "Warning: you are running few-shot learning methods on `supervised` dataset, if it is not expected, please change to `--mode inter` or `--mode intra`.")
    ...
    if model_name == 'proto':
        ...
        model = NNShot(word_encoder, dot=opt.dot, ignore_index=opt.ignore_index)
        framework = FewShotNERFramework(train_data_loader, val_data_loader, test_data_loader, N=opt.N, tau=opt.tau,
                                        train_fname=opt.train, viterbi=True, use_sampled_data=opt.use_sampled_data)
    else:
