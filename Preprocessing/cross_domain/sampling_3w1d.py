...
prepath = '../../../project_data/DocEE/normal_setting/3w1d/'
nlp = spacy.load("en_core_web_sm")

split = 'test' # 'train, 'dev', 'test'
if split != 'train':
    lengh_control = 500000
else:
    lengh_control = 50 # control sampling length
...
with open( prepath + split + '_com_3w1d.pkl', 'rb') as f:
...
def get_event_key_instance(new_train, event, key, comb_list):
    ...
    for p1 in comb_list:
        ...
        if set(labels1 ) == set(picked_comb + ['O']) and len(set(labels1 )) == 4:
            word = [tokens1]
            label = [labels1]
...
with jsonlines.open( prepath + split + '_3w1d.jsonl', 'w') as writer:
    writer.write_all(res_final)

# python sampling_3w1d.py