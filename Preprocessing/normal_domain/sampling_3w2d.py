...
prepath = '../../../project_data/DocEE/normal_setting/3w2d'
...
if split != 'train':
    ...
    lengh_control = 10 # control sampling length
...
with open( prepath + '3w1d/' + split + '_com_3w1d.pkl', 'rb') as f:
...
def get_event_key_instance(new_train, event, key, comb_list):
    ...
    for p1 in comb_list:
        ...
        if set(labels1 + labels2) == set(picked_comb + ['O']) and len(set(labels1 + labels2)) == 4:
    ...
    for sup, que in chocie:
        ...
        if set(s_l) == set(q_l) and len(combed_type) == 3:
...
with jsonlines.open( prepath + split + '_3w2d.jsonl', 'w') as writer:
    writer.write_all(res_final)

# python sampling_3w2d.py